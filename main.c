...
static inline void print_footer(duda_request_t *dr)
{
    response->printf(dr, "\
...
void cb_list_rows(duda_request_t *dr)
{
    ...
    sprintf(use_db_query, "USE %s", db);
    mariadb->query(conn, use_db_query, NULL, NULL, NULL, NULL);
    sprintf(select_query, "SELECT * FROM %s LIMIT %ld, %d", table, (page - 1) * page_size, page_size);
    mariadb->query(conn, select_query, NULL, cb_ls_row, cb_ls_end, (void *) row_array);
    mariadb->disconnect(conn, cb_on_disconnect);
}
...
void cb_home_page(duda_request_t* dr)
{
  ...
  <div class=\"container\">\
    <div class=\"jumbotron\">\
      ...
      <p><a class=\"btn btn-primary btn-large\" href=\"#\">Get Started »</a></p>\
    </div>\
  </div>");
    print_footer(dr);
    response->end(dr, NULL);
